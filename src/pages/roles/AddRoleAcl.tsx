...
import type { RoleOption, ModuleOption } from '../../services/roleAcls.service';
import { Alert, showAlert } from '../../components/common';
import { extractErrorMessage } from '../../utils/formatters';

interface RoleAclFormData {
...
const AddRoleAcl = () => {
    ...
    const fetchOptions = async () => {
      try {
        const [rolesRes, modulesRes] = await Promise.all([
          roleAclsService.getRoles(),
          roleAclsService.getModules(),
        ]);

        if (rolesRes.success && rolesRes.data) {
          const rows = Array.isArray(rolesRes.data) ? rolesRes.data : rolesRes.data.rows;
          setRoles(rows || []);
        }
  ...
  const onSubmit = async (data: RoleAclFormData) => {
    ...
    try {
      const response = await roleAclsService.addRoleAcl(
        {
      ...
      if (response.success) {
        setSuccessMessage('Role ACL added successfully');
        showAlert('success-alert');
        setTimeout(() => {
          navigate('/dashboard/roles/acls');
        }, 1500);
      } else {
        setError(response.message || 'Failed to add role ACL');
        showAlert('error-alert');
      }
    } catch (err: any) {
      setError(extractErrorMessage(err, 'Failed to add role ACL'));
      showAlert('error-alert');
    } finally {
      setLoading(false);
    }
  ...
  if (loadingOptions) {
    return (
      <div>
        <Navbar title="Add Role ACL" subtitle="Grant module access to a role" />
  ...
  return (
    <div>
      ...
      <div className="xui-py-1">
        ...
        <p className="xui-font-sz-[16px] xui-opacity-4">Select a role and module, then configure the permissions to grant.</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="xui-form">
          <div className="xui-d-grid xui-grid-col-1 xui-md-grid-col-2 xui-grid-gap-2">
            <div>
              <div className="xui-form-box">
                ...
                <select
                  id="role_unique_id"
                  {...register('role_unique_id', { required: 'Role is required' })}
                >
          ...
          >
            {loading ? 'Adding Role ACL...' : 'Add Role ACL'}
          </button>
        </form>
      </div>
