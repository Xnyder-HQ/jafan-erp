...
const AddMachine = () => {
  ...
  const onSubmit = async (data: MachineFormData) => {
    ...
    try {
      const payload = {
        ...
        ...(data.expected_blocks_per_day && { expected_blocks_per_day: Number(data.expected_blocks_per_day) }),
        ...(data.installed_date && { installed_date: data.installed_date }),
      };

      const response = await machinesService.addMachine(payload, {
      ...
      if (response.success) {
        ...
        setError(response.message || 'Failed to add machine');
        showAlert('error-alert');
      }
    } catch (err: any) {
      setError(extractErrorMessage(err, 'Failed to add machine'));
      showAlert('error-alert');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      <Navbar title="Add Machine" subtitle="Create a new machine" />
      <div className="xui-py-1">
        ...
        <p className="xui-font-sz-[16px] xui-opacity-4">Fill in the machine details below. Fields marked with * are required.</p>
