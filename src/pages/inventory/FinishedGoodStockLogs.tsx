...
import finishedGoodStockLogsService from '../../services/finishedGoodStockLogs.service';
import type { FinishedGoodStockLog } from '../../services/finishedGoodStockLogs.service';
...
const FinishedGoodStockLogs = () => {
  const { getAccessIds } = useGeneral();
  const [searchQuery, setSearchQuery] = useState('');
  const [dateFilter, setDateFilter] = useState<DateRange | null>(null);
  const [logs, setLogs] = useState<FinishedGoodStockLog[]>([]);
  ...
  const fetchLogs = useCallback(async () => {
    ...
    setFetchError('');
    try {
      const response = await finishedGoodStockLogsService.getStockLogs({
        page: currentPage,
        size: pageSize,
  ...
  const searchLogs = useCallback(async (query: string) => {
    ...
    setFetchError('');
    try {
      const response = await finishedGoodStockLogsService.searchStockLogs({
        search: query,
        page: currentPage,
        size: pageSize,
        module_unique_id: moduleId,
  ...
  const filterLogs = useCallback(async (range: DateRange) => {
    ...
    setFetchError('');
    try {
      const response = await finishedGoodStockLogsService.filterStockLogs({
  ...
  return (
    <div>
      <Navbar title="Finished Good Stock Logs" subtitle="Track finished good stock movements and changes" />
      ...
      <div className="xui-py-1-half">
        <div className="xui-d-flex xui-flex-ai-center xui-flex-jc-space-between xui-mb-1-half">
          ...
          <div className="xui-d-flex xui-flex-ai-center xui-grid-gap-half">
            ...
            <button
              onClick={() => modalShow('export-fg-stock-logs-modal')}
              className="xui-btn xui-btn-text xui-font-sz-80 xui-bdr-rad-half xui-font-w-500 xui-d-flex xui-flex-ai-center xui-grid-gap-half"
              style={{ border: '1px solid var(--neutral-300)', color: 'var(--neutral-700)' }}
                ...
                <tbody>
                  {logs.map((log) => (
                    <tr key={log.unique_id}>
                      <td>
                        <div>
                          <span className="xui-font-w-500">{log.FinishedGood?.name || 'N/A'}</span>
                          {log.FinishedGood?.reference && (
                            <p className="xui-font-sz-80 xui-opacity-5 xui-mt-half">{log.FinishedGood.reference}</p>
                          )}
                        </div>
                      </td>
                      <td>{getMovementBadge(log.movement_type)}</td>
                      <td className="xui-font-w-500">
                        {log.quantity.toLocaleString()} {log.FinishedGood?.unit_of_measure || ''}
                      </td>
                      <td>
                        {log.unit_cost != null ? (
                      ...
                      <td className="xui-font-w-500">
                        {log.quantity_after.toLocaleString()} {log.FinishedGood?.unit_of_measure || ''}
                      </td>
                      <td>
      ...
      <ExportModal
        id="export-fg-stock-logs-modal"
        title="Export Stock Logs"
        fileName="finished-good-stock-logs"
        columns={[
          { key: 'FinishedGood.name', header: 'Finished Good' },
          { key: 'FinishedGood.reference', header: 'Product Ref' },
...
export default FinishedGoodStockLogs;
